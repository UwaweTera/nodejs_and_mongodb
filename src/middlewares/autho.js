...
import { regVal, UserRegVal, roleVal, updateProfVal } from "../validater/regValidate";
...
const roleVald = async (req, res, next) => {
  const { error, value } = roleVal.validate(req.body, { abortEarly: false });
  if (error) {
    return res.status(400).json(error.details[0].message);
  }
  next();
...
const updateProfVald = async (req, res, next) => {
  const { error, value } = updateProfVal.validate(req.body, { abortEarly: false });
  if (error) {
    return res.status(400).json(error.details[0].message);
  }
  next();
...
export {  UserVald, adminVald, roleVald, updateProfVald };
